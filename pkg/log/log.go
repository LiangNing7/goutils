// Package log is a log package used by goutils project.
//
...
func WithContextExtractor(contextExtractors ContextExtractors) Option {
	return func(l *zapLogger) {
		// 如果还没有初始化，先分配好 map.
		if l.contextExtractors == nil {
			l.contextExtractors = make(ContextExtractors, len(contextExtractors))
		}
		// 直接完成所有键值拷贝.
		maps.Copy(l.contextExtractors, contextExtractors)
	}
}
