...
// NewStore create a `*Store` instance to handle token storage, deletion, and checking.
func NewStore(cfg *Config) *Store {
	// The reason `github.com/LiangNing7/goutils/pkg/db` is not used here is
	// to minimize dependencies, and use `github.com/redis/go-redis/v9` to
	// create redis client is not complex.
	// 此处未使用 `github.com/LiangNing7/goutils/pkg/db` 的原因
	// 在于旨在最小化依赖项，且直接通过
	// `github.com/redis/go-redis/v9`
	// 创建Redis客户端并不复杂。
